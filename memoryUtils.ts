  ...
  try {
    ...
    if (includeDate) {
      const query = startDate
        ? {
            sql: `SELECT id, date(date) as date, text FROM memories WHERE date >= :startDate AND date <= date(:startDate, '+7 days') ORDER BY date ASC`,
            args: { startDate },
          }
...
export function formatMemoriesForPrompt(memories) {
  ...
  const datedMemories = memories
    ...
    .map((memory) => {
      ...
      return `- ${date.toFormat("yyyy-MM-dd")} [ID: ${memory.id}]: ${
        memory.text
      }`;
    });
